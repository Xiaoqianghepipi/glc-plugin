...
const gachaCharacterDetailDir = path.join(pluginRoot, 'resources', 'gacha-character-detail')
const backgroundDir = path.join(pluginRoot, 'resources', 'common', 'img', 'background', 'default')
...
function getBackgroundImageList() {
  try {
    ...
    return fs.readdirSync(backgroundDir)
      ...
      .map((file) => `common/img/background/default/${file}`)
  } catch (err) {
...
function pickRandomBackground(backgroundList) {
  if (!Array.isArray(backgroundList) || !backgroundList.length) {
    return 'common/img/background/default/background.jpg'
  }

  const index = Math.floor(Math.random() * backgroundList.length)
  return backgroundList[index] || 'common/img/background/default/background.jpg'
}
...
function getFolderImagePath(characterName) {
  ...
  try {
    ...
    return picked ? `gacha-character-detail/${characterName}/${picked}` : ''
  } catch (err) {
...
function getLegacyImagePath(characterName) {
  ...
  try {
    ...
    return matched ? `gacha-character-detail/${matched}` : ''
  } catch (err) {
    logger.warn('[归龙潮插件] 读取角色详情图片目录失败:', err)
    return ''
  }
