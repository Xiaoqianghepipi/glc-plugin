...
export default async function runtimeRender(e, path, renderData = {}, cfg = {}) {
  ...
  const renderScalePercent = Math.round(scale * 100)
  const renderDataWithScale = {
    ...renderData,
    renderScale: scale,
    renderScalePercent,
    scaleStyle: `style="transform: scale(${scale}); transform-origin: top left; width: calc(100% / ${scale});"`,
  ...
  return puppeteer.screenshot(path, renderDataWithScale)
}