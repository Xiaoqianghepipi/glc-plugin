...
function quartzToNodeSchedule(cron) {
  ...
  const normalized = [sec, min, hour, dayOfMonth, month, dayOfWeek]
    .map((part) => (part === '?' ? '*' : part))
...
export function refreshGachaSchedule() {
  ...
  const scheduleCron = quartzToNodeSchedule(cron)
  ...
  if (!cron || !scheduleCron) {
    stopJob()
    currentCron = ''
    return { ok: false, message: 'Cron 不能为空，且必须是 Quartz 7 段表达式。' }
  }
  ...
  stopJob()
  gachaJob = schedule.scheduleJob(scheduleCron, runAutoFetch)
